# kanidm_community.kanidm run Role
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: kanidm_community.kanidm.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from kanidm_community.kanidm
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: kanidm_community.kanidm.run
      vars:
        run_x: 42
```
